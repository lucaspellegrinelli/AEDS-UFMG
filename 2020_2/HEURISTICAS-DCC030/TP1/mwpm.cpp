...
edge_list find_min_weight_perfect_matching(vertex_list odd_degrees, graph_matrix edges){
  ...
  // Yes, O(n^3) is bad but it is only being ran on n = [number
  // of odd degree vertices], so it is less terrible. You can also
  // tweek this so the first 'for' runs less times, maybe something
  // like 32 or 64 times is enough to generate a great improvement.
  for(size_t c = 0; c < matching.size(); c++){
    for(size_t i = 0; i < matching.size(); i++){
      for(size_t j = 0; j < matching.size(); j++){
        // Stores the distance of each permutation
